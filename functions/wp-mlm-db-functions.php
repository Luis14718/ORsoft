...
function wpmlm_setLevel($depth) {
    ...
    if ($level_depth < $depth) {
        ...
        for ($j = $level_depth + 1; $j <= $depth; $j++) {
            $sql1 = "INSERT INTO {$table_prefix}wpmlm_level_commission(level_no) VALUES('" . $j . "')";
            $wpdb->query($sql1);
        }
    } else {
...
/* get user Tax details with wordpress users by user id */

function wpmlm_get_comment_history($userid) {
    global $wpdb;
    $table_prefix = $wpdb->prefix;
    $sql = "SELECT * FROM {$table_prefix}comment_history WHERE user_id = '" . $userid . "'";
    $results = $wpdb->get_results($sql);
    return $results;
}
...
function wpmlm_buildTree1(array $elements, $parentId) {
    static $counter = 0;
    ++$counter;
    $tree = array();
    foreach ($elements as $element) {
        ...
        if ($element->user_parent_id == $parentId) {
            $children = wpmlm_buildTree1($elements, $element->user_ref_id);
            if ($children) {
                $tree[] = $children;
            }
...
function wpmlm_update_user_dash($new_page_id) {
    global $wpdb;
    $table_prefix = $wpdb->prefix;
    $sql = "UPDATE {$table_prefix}wpmlm_general_information SET `user_dash` = '".$new_page_id."' WHERE `id` = 1 ";
    $results = $wpdb->query($sql);
    return $results;
}

function wpmlm_update_reg_form($new_page_id) {
    global $wpdb;
    $table_prefix = $wpdb->prefix;
    $sql = "UPDATE {$table_prefix}wpmlm_general_information SET `user_registration` = '".$new_page_id."' WHERE `id` = 1 ";
    $results = $wpdb->query($sql);
    return $results;
}
...
function wpmlm_insert_leg_amount_new($user_id, $package_amount,$package_id) {
    ...
    foreach ($result as $res) {
        $i++;
        // $level_percentage = wpmlm_get_level_percentage($i);
        $level_percentage = wpmlm_get_level_percentage_reg_pack($i,$package_id->id);
...
function wpmlm_getEwalletAmountByUser($type, $user_id) {
    global $wpdb;
    $table_name = $wpdb->prefix . "wpmlm_ewallet_history";
    $sql = "SELECT ROUND(SUM(`amount`),2) as sum FROM {$table_name} where `type` = '$type' AND `user_id`='$user_id' ";
    $result = $wpdb->get_row($sql);
    return $result;
}
...
function wpajax_delete_comment($did){
    global $wpdb; 
    $table_name = $wpdb->prefix.'comment_history';
    $delete_data = $wpdb->delete( $table_name, array( 'id' => $did ) );
    if ($delete_data) {
     return true;
    }
    else{
...
function get_single_comment($id){
    global $wpdb;
    $table_name = $wpdb->prefix . "comment_history";
    $sql2 = "SELECT * FROM {$table_name} WHERE `id`='".$id."'";
    return $result = $wpdb->get_row($sql2);
}

function get_user_comments($user_id){
    global $wpdb;
    $table_name = $wpdb->prefix . "comment_history";
    $sql2 = "SELECT * FROM {$table_name} WHERE `user_id`='".$user_id."' AND `status`='0' ORDER BY `id` DESC";
    
    return $result = $wpdb->get_results($sql2);
}

function upload_vault_doc($document){
    global $wpdb;
    $table_name = $wpdb->prefix . "document_vault";
    $upload = $wpdb->insert($table_name, $document);
     if ($upload) {
        return true;
    }
    else{
...
function get_all_vault(){
    global $wpdb;
    $table_name = $wpdb->prefix . "document_vault";
    $sql3 = "SELECT * FROM {$table_name} ORDER BY `id` DESC";
    return $result = $wpdb->get_results($sql3);
}

function get_user_vault($username){
    global $wpdb;
    $table_name = $wpdb->prefix . "document_vault";
    $sql3 = "SELECT * FROM {$table_name} WHERE `username`='".$username."' ORDER BY `id` DESC";
    return $result = $wpdb->get_results($sql3);
}

//Level commission base on package
function wpmlm_update_table_column_levelcommission($id){
    $package = "package_reg".$id;
	global $wpdb;
	$table_name = $wpdb->prefix . 'wpmlm_level_commission';
    $wpdb->query("ALTER TABLE {$table_name} ADD  $package int(11) NOT NULL DEFAULT 0");
    return true;
}
function wpmlm_delete_table_column_levelcommission($id){
    $package = "package_reg".$id;
	global $wpdb;
	$table_name = $wpdb->prefix . 'wpmlm_level_commission';
    $wpdb->query("ALTER TABLE {$table_name} DROP COLUMN  $package");
    return true;
}

function wpmlm_update_reg_pack_level_commission($level_commission) {
    
    global $wpdb;
    $table_prefix = $wpdb->prefix;   
    $array_key = array_keys($level_commission);
    $j = 0;
    $k = 0;
    ...
    foreach ($level_commission as $com) { 
        ...
        foreach($com as $commission){
            $sql = "UPDATE {$table_name} SET  $array_key[$k]= '" . $commission . "' where `level_no`= $m  ";
            $wpdb->query($sql);
            $m++;
        }
        $k++;
...
function wpmlm_ifactiveUser($username){
    global $wpdb;
