...
function wpmlm_commission_settings() {
    ...
    <div id="level-settings">
        ...
        <div class="panel-heading">
            <h4><i class="fa fa-external-link-square"></i> <span> <?php _e('Bonus Settings','wpmlm-unilevel'); ?></span></h4>
            
        </div>
        <div class="panel-border">
            <h5>Rank Settings</h5>
            <div class="submit_message"></div>
            <form id="rank-form" class="form-horizontal" method="post">
                <div class="form-group">
                    <label class="control-label col-md-3 user-dt" for="rankdepth">Rank Bonus Depth:</label>
                    <div class="col-md-6">
                        <input type="number" class="form-control" name="rankdepth" id="rankdepth" placeholder="<?php _e('Enter Level','wpmlm-unilevel'); ?>" value="<?php echo count($level_commission);?>" min="0" onkeypress="return isNumberKey(event)">
                    </div>
                </div>
                ...
                <div class="form-group"> 
                    <div class="col-sm-offset-3 col-sm-6">
                        ...
                        <input type="submit" name="depth-submit" class="btn btn-danger" value="<?php _e('Save','wpmlm-unilevel'); ?>">
                    </div>
                </div>
            </form> 
        </div>
        <div class="panel-border">
            <h5><?php _e('Level Settings','wpmlm-unilevel'); ?></h5>
            <div class="submit_message"></div>
            <form id="depth-form" class="form-horizontal" method="post">
                <div class="form-group">
                    <label class="control-label col-md-3 user-dt" for="depth"><?php _e('Level Bonus Depth','wpmlm-unilevel'); ?>:</label>
                    <div class="col-md-6">
                        <input type="number" class="form-control" name="depth" id="depth" placeholder="<?php _e('Enter Level','wpmlm-unilevel'); ?>" value="<?php echo count($level_commission);?>" min="0" onkeypress="return isNumberKey(event)">
                    </div>
                </div>
                <div class="form-group">
                    <label class="control-label col-md-3 user-dt" for="depth_agree"><?php _e('I agree','wpmlm-unilevel'); ?></label>
                    <div class="col-md-6">
        ...
        <div class="panel-border">  
            <h5><?php _e('Bonus Settings','wpmlm-unilevel'); ?> </h5>
                ...
                <div class="commission-inner-div">
                    <?php if (count($level_commission) > 0) { ?>
                            <div class="col-md-12" style="margin-bottom: 10px">
                                <input  type="radio" value="percentage" name="level_type" <?php echo($res->level_commission_type == 'percentage' ) ? 'checked' : ''; ?>  > <span> <?php _e('Percentage','wpmlm-unilevel'); ?>&nbsp;&nbsp;</span>
                                <input type="radio" value="flat" name="level_type" <?php echo ($res->level_commission_type == 'flat') ? 'checked' : ''; ?> > <span> <?php _e('Flat','wpmlm-unilevel'); ?></span>
                            <!--</div>-->
                </div>
 ...
 -->        </div>
    ...
    <script>
        jQuery(document).ready(function ($) {
            ...
            $("#rank-form").submit(function () {                
                $(".submit_message").html('');
                $(".submit_message").show();
                isValid = true;
                if (($("#rankdepth").val() == '') || ($("#rankdepth").val() < 1)) {
                    
                    $("#rankdepth").addClass("invalid");
                    isValid = false;
                    return false;
                } else if (isValid) {
                    ...
                    $.ajax({
                        type: "POST",
                        url: ajaxurl,
                        data: {depth: $("#rankdepth").val(),action:'wpmlm_level_rank_bonus'},
                        cache: false,
