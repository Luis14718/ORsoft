...
function create_wpmlm_users_table() {
    ...
    `active_inactive` varchar(50) CHARACTER SET utf8 NOT NULL DEFAULT 'active',
    PRIMARY KEY (`user_id`)
                          ) $charset_collate AUTO_INCREMENT=1";

      require_once( ABSPATH . 'wp-admin/includes/upgrade.php' );
      dbDelta($sql);
      add_option( 'mlm_db_version', $mlm_db_version );
}
...
");
    ...
    //$wpdb->query("DROP TABLE {$table_prefix}tax_details");
    $wpdb->query("DROP TABLE {$table_prefix}comment_history");
    $wpdb->query("DROP TABLE {$table_prefix}document_vault");
}
...
function create_wpmlm_rank_commission_table() {
    global $wpdb;
    $table_prefix = $wpdb->prefix;
    $charset_collate = $wpdb->get_charset_collate();
    $sql = "CREATE TABLE {$table_prefix}wp_wpmlm_rank_commission (
    `ID` int(11) NOT NULL AUTO_INCREMENT,
    ...
    require_once( ABSPATH . 'wp-admin/includes/upgrade.php' );
    dbDelta($sql);
    add_option( 'wpmlm_rank_commission_db_version', '1.0' );
}
...
function insert_wpmlm_paypal_data() {
    global $wpdb;
...
function create_wpmlm_general_information_table() {
    ...
    `user_dash` int(11) NOT NULL DEFAULT '0',
    `user_registration` int(11) NOT NULL DEFAULT '0',
...
function create_wpmlm_comment_table(){

    global $wpdb;
    $table_prefix = $wpdb->prefix;
    $charset_collate = $wpdb->get_charset_collate();
    $sql = "CREATE TABLE IF NOT EXISTS {$table_prefix}comment_history (
    `id` int(20) NOT NULL AUTO_INCREMENT,
...
function create_wpmlm_document_vault(){
    global $wpdb;
    $table_prefix = $wpdb->prefix;
    $charset_collate = $wpdb->get_charset_collate();
    $sql = "CREATE TABLE IF NOT EXISTS {$table_prefix}document_vault (
    `id` int(20) NOT NULL AUTO_INCREMENT,
    `username` varchar(20) NOT NULL DEFAULT '0',
    `caption` varchar(50) NOT NULL DEFAULT '0',
...
function new_install($mlm_db_version){
    global $wpdb;
    $installed_ver = get_option( "mlm_db_version" );

    if ( $installed_ver != $mlm_db_version ) {
        if(version_compare($mlm_db_version, '1.0.1') == -1){
	    // $table_name = $wpdb->prefix . 'wpmlm_users';

	    // $sql = "ALTER TABLE {$table_name} DROP COLUMN  `inactive_active`";
     //    $wpdb->query($sql);

	    update_option( "mlm_db_version", $mlm_db_version );
        }
    }
}